...
class Runner_MAPPO_MPE:
    def __init__(self, args, env_name, number, seed):
        ...
        self.args.obs_dim_n = [
            {
                "individual": self.env.observation_space[i]["individual"].shape[0],
        ...
        self.args.action_dim_n = [
            {
                "thresholds": self.env.action_space[i]["thresholds"].shape[0],
    ...
    def run_episode_mpe(self, evaluate=False):
        ...
        for episode_step in range(self.args.episode_limit):
            ...
            obs_n = [np.append(item["individual"], item["group"]) for item in obs_n]
        ...
        self.store += r_n[0]
        self.writer.add_scalar(
            "step_reward_{}".format(self.env_name),
            r_n[0],
            global_step=self.total_steps + episode_step,
        )
        self.writer.add_scalar(
            "episode_reward_{}".format(self.env_name),
            self.store,
            global_step=self.total_steps + episode_step,
        )
