...
class Environment(gym.core.Env):
        ...
        self.observation_space = gym.spaces.Dict(
            {
                i: gym.spaces.Dict(
                    {
                        "individual": gym.spaces.Box(
                            low=0, high=10, shape=(5, 2), dtype=float
                        ),
                        "group": gym.spaces.Box(
                            low=0, high=10, shape=(5, 2), dtype=float
                        ),
                    }
                )
    ...
    def calc_group_rank(self, p):
        ...
        observation = [
            {"individual": self.first_ranking[i], "group": group_rank}
            for i in range(self.n_member)
        ]
        return observation
